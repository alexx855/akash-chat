    ...
    return (
      <div
        className={`group px-4 ${message.role === 'assistant'
          ? 'border-b border-black/10 bg-gray-50 text-gray-800 dark:border-gray-900/50 dark:bg-[#333d3d3d] dark:text-gray-100'
          : 'border-b border-black/10 bg-white text-gray-800 dark:border-gray-900/50 dark:bg-[#331c1c1c] dark:text-gray-100'
          }`}
        ...
        <div className="relative m-auto flex gap-4 p-4 text-base md:max-w-2xl md:gap-6 md:py-6 lg:max-w-2xl lg:px-0 xl:max-w-3xl">
          <div className="min-w-[40px] text-right font-bold">
            {message.role === 'assistant' ? (
              message.toolInvocations ? (<IconTools size={30} />) : (<IconRobot size={30} />)
            ) : (
              <IconUser size={30} />
            )}
          </div>
          ...
          <div className="prose mt-[-2px] w-full dark:prose-invert">
            {message.role === 'user' ? (
              <div className="flex w-full">
                {isEditing ? (
                  <div className="flex w-full flex-col">
                    <textarea
                      ref={textareaRef}
                      className="w-full resize-none whitespace-pre-wrap border-none dark:bg-[#331c1c1c]"
              ...
              <>
                {message.toolInvocations ? message.toolInvocations.map((toolInvocation: ToolInvocation) => {
                  ...
                  return 'result' in toolInvocation ? (
                    ...
                    ) : toolInvocation.toolName === 'generateAudio' ? (
                      <div key={toolCallId} className="">
                        <audio
                          controls
                          src={toolInvocation.result}
